...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  app.get('/api/entries', async (c) => {
    const { cursor, limit, contentTypeId } = c.req.query();
    const all = await ContentEntryEntity.list(c.env, cursor, limit ? parseInt(limit) : undefined);
    if (contentTypeId) {
      all.items = all.items.filter(it => it.contentTypeId === contentTypeId);
    }
    return ok(c, all);
  });
